...
const ProIdIncrease = urlparams.get("id");
console.log(ProIdIncrease);
...
function updatePrice() {
  ...
  icrementbtn.onclick = function () {
      ...
      const proArr=JSON.parse(localStorage.getItem('Products'));
    const index=proArr.findIndex(x=>x.id===ProIdIncrease)
    let TargetPro=proArr[index]
    if(TargetPro.quantity==counterval){
        Swal.fire({
        icon: 'error',
        title: "Stock!!",
...
ProCart=storeddata;
    ...
    document.getElementById('product-price').textContent = (storeddata.EndPrice * counterval) + " EGP";
if(storeddata.quantity<1){
  document.getElementById('addToCartBtnDetails').classList.add('d-none')
